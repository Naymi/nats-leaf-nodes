...
const main = async () => {
  try {
    ...
    const { agentNc, agentJs } = await createAgentConnect();
    ...
    setInterval(async () => {
      ...
      await agentJs.publish(subject, sc.encode(cmdName));
    ...
    await agentJs.consumers.get(cmdResultStreamName, mainResultConsumer).catch(() => null) ?? await jsm.consumers.add(cmdResultStreamName, {
